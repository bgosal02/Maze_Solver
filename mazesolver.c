/*
 File:    mazesolver.c
 Purpose: A classic maze-solver that uses recursion.
...
maze_cell** parse_maze(FILE* maze_file, int dimension)
{
	...
	for (row = 0; row < dimension; ++row) {
		maze[row] = (maze_cell*)calloc(row, sizeof(maze_cell));
	}
...
void generate_all_paths(char*** pathsetref, int* numpathsref, maze_cell** maze, int dimension, int row, int column, char* path)
{
	...
	else if (maze[row][column].visited == VISITED || maze[row][column].character == MAZE_WALL)
	{
		return;
	}
...
int path_cost(char* path_string)
{
	if (*path_string == '\0') {
		...
		int num = *path_string - '0';
		return num + path_cost(path_string + 1);
	}
}
...
{
	char* shortest_path = *pathset;
	int shortest_path_length = strlen(*pathset);
	int cmp_length = 0;

	for (int i = 1; i < numpaths; i++) {
		cmp_length = strlen(*(pathset + i));

		if (cmp_length < shortest_path_length) {
			shortest_path_length = cmp_length;
			shortest_path = *(pathset + i);
		}
	}

	sprintf(outputbuffer, "Shortest path: %s", shortest_path);
}
...
{
	char* cheapest_path = *pathset;
	int cheapest_path_cost = path_cost(*pathset);
	int cmp_cost = 0;

	for (int i = 1; i < numpaths; i++) {
		cmp_cost = path_cost(*(pathset + i));

		if (cmp_cost < cheapest_path_cost) {
			cheapest_path_cost = cmp_cost;
			cheapest_path = *(pathset + i);
		}
	}

	sprintf(outputbuffer, "Cheapest path: %s\nCheapest path cost: %d", cheapest_path, cheapest_path_cost);
}
